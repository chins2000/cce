...
<body>
	...
	<script type="text/javascript">
		...
		$("#searchbtn").on("click", function() {
			...
			$thefrom.attr("action","<%=path%>/user/search");
			$thefrom.submit();
		});
		$("#pageContains > div").each(function(){
			$(this).on("click", function() {
				if($(this).attr("disabled")=="disabled")
					return false;
				$thefrom = $($(".form-horizontal")[0]);
				$thefrom.attr("action","<%=path%>/user/searchpage");
		...
		$("span[title='添加']").on("click", function(){
			top.openDialog("<%=path%>/user/toadd");
		});
		$("span[title='修改']").on("click", function(){
			top.openDialog("<%=path%>/user/toupdate?id="+$(this).attr('data-id'));
		});
		$("span[title='删除']").on("click", function(){
			$.post("<%=path%>/user/delete",{"id":$(this).attr("data-id")});
		});
		$("span[title='删除选中项']").on("click", function(){
			...
			$.post("<%=path%>/user/deleteMore",{"ids":ids});
		});

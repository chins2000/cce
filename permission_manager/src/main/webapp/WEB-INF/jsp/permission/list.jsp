...
<body>
	...
	<script type="text/javascript">
		...
		$("#searchbtn").on("click", function() {
			...
			$thefrom.attr("action","<%=path%>/permission/search");
			$thefrom.submit();
		});
		$("#pageContains > div").each(function(){
			$(this).on("click", function() {
				if($(this).attr("disabled")=="disabled")
					return false;
				$thefrom = $($(".form-horizontal")[0]);
				$thefrom.attr("action","<%=path%>/permission/searchpage");
		...
		$("span[title='添加']").on("click", function(){
			top.openDialog("<%=path%>/permission/toadd");
		});
		$("span[title='修改']").on("click", function(){
			top.openDialog("<%=path%>/permission/toupdate?id="+$(this).attr('data-id'));
		});
		$("span[title='删除']").on("click", function(){
			$.post("<%=path%>/permission/delete",{"id":$(this).attr("data-id")});
		});
		$("span[title='删除选中项']").on("click", function(){
			...
			$.post("<%=path%>/permission/deleteMore",{"ids":ids});
		});

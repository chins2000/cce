...
<body>
	...
	<script type="text/javascript">
		...
		$("#searchbtn").on("click", function() {
			...
			$thefrom.attr("action","<%=path%>/menu/search");
			$thefrom.submit();
		});
		$("#pageContains > div").each(function(){
			$(this).on("click", function() {
				if($(this).attr("disabled")=="disabled")
					return false;
				$thefrom = $($(".form-horizontal")[0]);
				$thefrom.attr("action","<%=path%>/menu/searchpage");
		...
		$("span[title='添加']").on("click", function(){
			top.openDialog("<%=path%>/menu/toadd");
		});
		$("span[title='修改']").on("click", function(){
			top.openDialog("<%=path%>/menu/toupdate?id="+$(this).attr('data-id'));
		});
		$("span[title='删除']").on("click", function(){
			$.post("<%=path%>/menu/delete",{"id":$(this).attr("data-id")});
		});
		$("span[title='删除选中项']").on("click", function(){
			...
			$.post("<%=path%>/menu/deleteMore",{"ids":ids});
		});
